...
export default function Home() {
  const [apiData, setApiData] = useState<any>(null);  // 用于存储从 API 返回的数据
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("http://localhost:8000/api/v1/articles?article_id=67beacaa816f520c793767d2");
      if (!response.ok) {
  ...
  return (
    <div className="grid grid-rows-[20px_1fr_20px] items-center justify-items-center min-h-screen p-8 pb-20 gap-16 sm:p-20 font-[family-name:var(--font-geist-sans)]">
      <div className="flex gap-4 items-center flex-col sm:flex-row">
        ...
        {apiData && apiData.content && (
          <div className="mt-4 p-4 border border-gray-200 rounded">
            <h3 className="font-bold">Article Content:</h3>
            <p>{apiData.title}</p> {/* 渲染 content 属性 */}
          </div>
        )}
      </div>
    </div>
